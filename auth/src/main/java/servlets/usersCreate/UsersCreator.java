...
public class UsersCreator {
	...
	public static void createUser(HttpServletRequest req, HttpServletResponse resp,
		...
		try (Connection conn = dbAuth.getConnection()) {
			...
			User user = new User.Builder(user_uuid, email, time, time)
					.passwordHash(PassUtil.sha256Hash(pass)).fullName(FullName).accType("password")
			...
			EmailToken emailToken = new EmailToken(token, user_uuid,
					(System.currentTimeMillis() / 1000L) + 86_400);
			...
			String verifyLink = dbAuth.BACK_HOST + ApiConstants.USERS_VERIFY_EMAIL + "?token=" + token
					+ "&redirect=" + redirectUrl;
			MailUtil.sendMail(email, "Verify your E-Mail for HridayKh.in",
